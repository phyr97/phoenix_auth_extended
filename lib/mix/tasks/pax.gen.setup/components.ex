defmodule Mix.Tasks.Pax.Gen.Setup.Components.Docs do
  @moduledoc false

  def short_doc do
    "Configures core components for authentication"
  end

  def example do
    "mix pax.gen.setup.components"
  end
  ...
  def long_doc do
    ...
    This task modifies the core components for authentication:

    ## Component Updates
    * Adjusts simple_form spacing from space-y-8 to space-y-4 for better form layout
    ...
    * `lib/<app>_web/components/core_components.ex`
    ...
    The button_link component is only added when using OAuth or Passkey authentication.
    It provides a consistent button-like appearance for links while maintaining proper
    semantic HTML by using <a> tags instead of <button> elements.

    The simple_form spacing adjustment improves the visual rhythm of form elements
    by reducing the gap between fields from 2rem to 1rem.
    """
  end
end

if Code.ensure_loaded?(Igniter) do
  defmodule Mix.Tasks.Pax.Gen.Setup.Components do
  ...
  defmodule Mix.Tasks.Pax.Gen.Setup.Components do
    ...
    def run(_argv) do
      Mix.shell().error("""
      The task 'pax.gen.setup.components' requires igniter. Please install igniter and try again.
