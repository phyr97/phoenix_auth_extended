defmodule PhoenixAuthExtendedTestWeb.PasskeyRegistrationLive do
  @moduledoc """
  ...
  def handle_info({:registration_successful, params}, socket) do
    ...
    with {:ok, %User{} = user} <- Accounts.register_user(user_attrs),
         {:ok, %UserToken{value: token}} <- Accounts.generate_user_session_token(user) do
